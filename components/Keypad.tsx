...
const KeypadButton: React.FC<{ onClick: () => void; children: React.ReactNode; className?: string }> = ({ onClick, children, className = '' }) => (
    <button
        onClick={onClick}
        className={`bg-gray-700 hover:bg-gray-600 active:bg-gray-500 rounded-lg text-3xl font-semibold transition-colors duration-150 aspect-square flex items-center justify-center ${className}`}
    >
        {children}
    </button>
);
